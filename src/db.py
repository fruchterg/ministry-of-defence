...
class DBTable(db_api.DBTable):
        ...
        with shelve.open(f"db_files/{self.name}_{critery.field_name}", writeback=True) as db_index:
            for key in db_index.keys():
                try:
                    ...
                    print("invalid Name")
        return is_critery
    def is_query_exist(self,data_table, list_keys, list_criteria):
    ...
    def query_table(self, criteria: List[SelectionCriteria]) -> List[Dict[str, Any]]:
        ...
        selection_criteria_list = self.is_query_exist(data_table, list_keys, criter_no_index)
        data_table.close()
        return selection_criteria_list
    ...
    def create_index(self, field_to_index: str) -> None:
        ...
        for record_in_table in data_table.keys():
            try:
                db_index[data_table[record_in_table][field_to_index]].append(record_in_table)
            except:
                db_index[data_table[record_in_table][field_to_index]] = [record_in_table]
